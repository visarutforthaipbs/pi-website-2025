...
import { isArray, zip, isObject, uniq, isEqual, mapValues, pick, reject as reject$1, capitalize, isEmpty, last, set, isNumber, get, has, pipe, omit, assign, map as map$1, size, isNil, clone, once, castArray, keyBy } from 'lodash/fp';
    ...
    async integrityCheck() {
        ...
        try {
            ...
            if (error instanceof TransferEngineValidationError && error.details?.details?.diffs) {
                ...
                if (isEmpty(_class_private_field_loose_base$6(this, _handlers)[_handlers].schemaDiff)) {
                    throw error;
                }
...
const omitInvalidCreationAttributes$1 = omit([
    'id'
]);
const createEntityQuery = (strapi)=>{
    ...
    const query = (uid)=>{
        ...
        const createMany = async (params)=>{
            ...
            .then(map$1((data)=>components.assignToEntity(uid, data)))// Remove unwanted attributes
            .then(map$1(omitInvalidCreationAttributes$1))// Execute a strapi db createMany query with all the entities + their created components
...
function startStepOnce(stage) {
    return once(()=>_class_private_field_loose_base$3(this, _startStep$1)[_startStep$1](stage));
}
...
export { index$7 as engine, index as file, index$2 as strapi, index$8 as utils };
