...
import path, { join, resolve as resolve$1, extname, basename, dirname } from 'path';
import _, { flatten, isNil as isNil$1, castArray as castArray$1, mergeWith as mergeWith$1, isArray as isArray$1, uniqBy, get as get$2, set, has as has$1, isString as isString$1, isNumber } from 'lodash';
import fp, { get as get$1, omit, isNil, isPlainObject, isObject, prop, has, toNumber, pipe, pick, isEmpty, negate, isEqual, isArray, castArray, trim, toLower, camelCase, cloneDeep, toString, isFunction, pickBy, defaultsDeep, mergeWith, merge, map, defaults as defaults$7, getOr, curry, assoc, assign, isString, keyBy, flatMap, values, sumBy } from 'lodash/fp';
import { createLogger } from '@strapi/logger';
import { errors as errors$1, Database } from '@strapi/database';
...
import fse, { existsSync as existsSync$1, pathExists } from 'fs-extra';
...
import assert$1 from 'assert/strict';
...
const getDirs = ({ appDir, distDir }, config)=>({
        ...
        static: {
            public: resolve$1(appDir, get$1('server.dirs.public', config))
        }
    });
...
var dependencies = {
    ...
    "@strapi/admin": "5.10.2",
    "@strapi/database": "5.10.2",
    "@strapi/generators": "5.10.2",
    "@strapi/logger": "5.10.2",
    "@strapi/permissions": "5.10.2",
    "@strapi/types": "5.10.2",
...
const isSingleType$1 = (contentType)=>contentTypes.isSingleType(contentType);
...
const publicKey = fs.readFileSync(resolve$1(__dirname, '../resources/key.pub'));
...
const policyOrMiddlewareSchema = yup$1.lazy((value)=>{
    if (typeof value === 'string') {
        return yup$1.string().required();
    }
    if (typeof value === 'function') {
        return yup$1.mixed().isFunction();
    }
    return yup$1.object({
        name: yup$1.string().required(),
        options: yup$1.object().notRequired()
    });
});
const routeSchema = yup$1.object({
    method: yup$1.string().oneOf([
        'GET',
        'POST',
        'PUT',
        'PATCH',
    ...
    path: yup$1.string().required(),
    handler: yup$1.lazy((value)=>{
        if (typeof value === 'string') {
            return yup$1.string().required();
        }
        if (Array.isArray(value)) {
            return yup$1.array().required();
        }
        return yup$1.mixed().isFunction().required();
    }),
    config: yup$1.object({
        auth: yup$1.lazy((value)=>{
            if (value === false) {
                return yup$1.boolean().required();
            }
            return yup$1.object({
                scope: yup$1.array().of(yup$1.string()).required()
            });
        }),
        policies: yup$1.array()// FIXME: fixed in yup v1
        .of(policyOrMiddlewareSchema).notRequired(),
        middlewares: yup$1.array()// FIXME: fixed in yup v1
        .of(policyOrMiddlewareSchema).notRequired()
    }).notRequired()
});
const validateRouteConfig = (routeConfig)=>{
    try {
        ...
        if (error instanceof yup$1.ValidationError) {
...
const middlewareConfigSchema = yup$1.array().of(yup$1.lazy((value)=>{
    if (typeof value === 'string') {
        return yup$1.string().required();
    }
    if (typeof value === 'object') {
        return yup$1.object({
            name: yup$1.string(),
            resolve: yup$1.string(),
            config: yup$1.mixed()
        }).required().noUnknown();
    }
    return yup$1.mixed().test(()=>false);
}));
/**
...
const lifecyclesShape = _.mapValues(_.keyBy(LIFECYCLES), ()=>yup$1.mixed().nullable().isFunction());
const contentTypeSchemaValidator = yup$1.object().shape({
    schema: yup$1.object().shape({
        info: yup$1.object().shape({
            displayName: yup$1.string().required(),
            singularName: yup$1.string().isKebabCase().required(),
            pluralName: yup$1.string().isKebabCase().required()
        }).required(),
        attributes: yup$1.object().test({
            name: 'valuesCollide',
    ...
    actions: yup$1.object().onlyContainsFunctions(),
    lifecycles: yup$1.object().shape(lifecyclesShape).noUnknown()
});
...
const { CREATED_AT_ATTRIBUTE, UPDATED_AT_ATTRIBUTE, PUBLISHED_AT_ATTRIBUTE, CREATED_BY_ATTRIBUTE: CREATED_BY_ATTRIBUTE$1, UPDATED_BY_ATTRIBUTE: UPDATED_BY_ATTRIBUTE$1 } = contentTypes.constants;
...
const strapiServerSchema = yup$1.object().shape({
    bootstrap: yup$1.mixed().isFunction(),
    destroy: yup$1.mixed().isFunction(),
    register: yup$1.mixed().isFunction(),
    config: yup$1.object(),
    routes: yup$1.lazy((value)=>{
        if (Array.isArray(value)) {
            return yup$1.array();
        }
        return yup$1.object();
    }),
    controllers: yup$1.object(),
    services: yup$1.object(),
    policies: yup$1.object(),
    middlewares: yup$1.object(),
    contentTypes: yup$1.object()
}).noUnknown();
...
const srcSchema = yup$1.object().shape({
    bootstrap: yup$1.mixed().isFunction(),
    register: yup$1.mixed().isFunction(),
    destroy: yup$1.mixed().isFunction()
}).noUnknown();
...
var loadSrcIndex = ((strapi)=>{
    ...
    const pathToSrcIndex = resolve$1(strapi.dirs.dist.src, 'index.js');
...
async function loadAPIs(strapi) {
    if (!existsSync$1(strapi.dirs.dist.api)) {
        return;
    }
...
const favicon = (config, { strapi })=>{
   ...
   */ if (!existsSync(resolve$1(appRoot, faviconPath))) {
        faviconPath = 'favicon.ico';
    }
    return koaFavicon(resolve$1(appRoot, faviconPath), {
        maxAge
    });
};
...
const bodyMiddleware = (config, { strapi })=>{
    const bodyConfig = defaultsDeep(defaults$3, config);
    let gqlEndpoint;
...
const security = (config, { strapi })=>(ctx, next)=>{
        let helmetConfig = defaultsDeep(defaults$1, config);
        const specialPaths = [
            '/documentation'
        ];
        const directives = {
...
const isStrapiPlugin = (info)=>get$1('strapi.kind', info) === 'plugin';
    ...
    if (declaration?.resolve) {
       ...
       */ pathToPlugin = join(declaration.resolve, '..');
            ...
            try {
                ...
                pathToPlugin = resolve$1(strapi.dirs.app.root, declaration.resolve);
...
async function loadPlugins(strapi1) {
    ...
    for (const pluginName of Object.keys(enabledPlugins)){
        ...
        plugins[pluginName] = {
            ...
            config: defaults$7(defaultPlugin.config, pluginServer.config),
...
const databaseErrorsToTransform$1 = [
    errors$1.InvalidTimeError,
    errors$1.InvalidDateTimeError,
    errors$1.InvalidDateError,
    errors$1.InvalidRelationError
];
/**
...
const defaultLocale = async (contentType, params)=>{
    ...
    if (!params.locale) {
        return assoc('locale', await getDefaultLocale$1(), params);
    }
    return params;
};
/**
...
const { isPolymorphic: isPolymorphic$1 } = relations;
/**
...
const textNodeValidator = yup$1.object().shape({
    ...
    text: yup$1.string().test('is-valid-text', 'Text must be defined with at least an empty string', (text)=>{
        return typeof text === 'string' || text === '';
    }),
    bold: yup$1.boolean(),
    italic: yup$1.boolean(),
    underline: yup$1.boolean(),
    strikethrough: yup$1.boolean(),
    code: yup$1.boolean()
});
...
const linkNodeValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'link'
    ]).required(),
    url: yup$1.string().test('invalid-url', 'Please specify a valid link.', (value)=>checkValidLink(value ?? '')),
    children: yup$1.array().of(textNodeValidator).required()
});
// TODO: remove any with a correct Type
const inlineNodeValidator = yup$1.lazy((value)=>{
    switch(value.type){
        case 'text':
        ...
        default:
            return yup$1.mixed().test('invalid-type', 'Inline node must be Text or Link', ()=>{
                return false;
            });
    }
});
const paragraphNodeValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'paragraph'
    ]).required(),
    children: yup$1.array().of(inlineNodeValidator).min(1, 'Paragraph node children must have at least one Text or Link node').required()
});
const headingNodeValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'heading'
    ]).required(),
    level: yup$1.number().oneOf([
        1,
        2,
        3,
        4,
    ...
    children: yup$1.array().of(inlineNodeValidator).min(1, 'Heading node children must have at least one Text or Link node').required()
});
const quoteNodeValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'quote'
    ]).required(),
    children: yup$1.array().of(inlineNodeValidator).min(1, 'Quote node children must have at least one Text or Link node').required()
});
const codeBlockValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'code'
    ]).required(),
    syntax: yup$1.string().nullable(),
    children: yup$1.array().of(textNodeValidator).min(1, 'Quote node children must have at least one Text or Link node').required()
});
const listItemNode = yup$1.object().shape({
    type: yup$1.string().equals([
        'list-item'
    ]).required(),
    children: yup$1.array().of(inlineNodeValidator).required()
});
// Allow children to be either a listItemNode or a listNode itself
// @ts-expect-error - listChildrenValidator needs a type
const listChildrenValidator = yup$1.lazy((value)=>{
    switch(value.type){
        case 'list':
        ...
        case 'list-item':
            return listItemNode;
        default:
            return yup$1.mixed().test('invalid-type', 'Inline node must be list-item or list', ()=>{
                return false;
            });
    }
});
// @ts-expect-error - listNodeValidator needs a type
const listNodeValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'list'
    ]).required(),
    format: yup$1.string().equals([
        'ordered',
        'unordered'
    ]).required(),
    children: yup$1.array().of(listChildrenValidator).min(1, 'List node children must have at least one ListItem or ListNode').required()
});
const imageNodeValidator = yup$1.object().shape({
    type: yup$1.string().equals([
        'image'
    ]).required(),
    image: yup$1.object().shape({
        name: yup$1.string().required(),
        alternativeText: yup$1.string().nullable(),
        url: yup$1.string().required(),
        caption: yup$1.string().nullable(),
        width: yup$1.number().required(),
        height: yup$1.number().required(),
        formats: yup$1.object().required(),
        hash: yup$1.string().required(),
        ext: yup$1.string().required(),
        mime: yup$1.string().required(),
        size: yup$1.number().required(),
        previewUrl: yup$1.string().nullable(),
        provider: yup$1.string().required(),
        provider_metadata: yup$1.mixed().nullable(),
        createdAt: yup$1.string().required(),
        updatedAt: yup$1.string().required()
    }),
    children: yup$1.array().of(inlineNodeValidator).required()
});
// TODO: remove the any and replace with a correct Type
const blockNodeValidator = yup$1.lazy((value)=>{
    switch(value.type){
        case 'paragraph':
        ...
        default:
            return yup$1.mixed().test('invalid-type', 'Block node is of invalid type', ()=>{
                return false;
            });
    }
});
const blocksValidatorSchema = yup$1.array().of(blockNodeValidator);
...
/* Type validators */ const stringValidator = (metas, options)=>{
    let schema = yup$1.string().transform((val, originalVal)=>originalVal);
...
const enumerationValidator = ({ attr })=>{
    return yup$1.string().oneOf((Array.isArray(attr.enum) ? attr.enum : [
        attr.enum
    ]).concat(null));
};
const integerValidator = (metas, options)=>{
    let schema = yup$1.number().integer();
 ...
 */ const buildRelationsStore = ({ uid, data })=>{
    ...
    return Object.keys(currentModel.attributes).reduce((result, attributeName)=>{
        ...
        switch(attribute.type){
            ...
            case 'media':
                {
                    ...
                    if (Array.isArray(value)) {
                        ...
                        if ('connect' in value && !isNil$1(value.connect)) {
                            source = value.connect;
                        } else if ('set' in value && !isNil$1(value.set)) {
                            source = value.set;
                        } else {
                            source = [];
                        }
                    } else {
                        source = castArray$1(value);
                    }
            ...
            case 'component':
                {
                    return castArray$1(value).reduce((relationsStore, componentValue)=>{
                        if (!attribute.component) {
                            throw new ValidationError$1(`Cannot build relations store from component, component identifier is undefined`);
                        }
                        return mergeWith$1(relationsStore, buildRelationsStore({
            ...
            case 'dynamiczone':
                {
                    return castArray$1(value).reduce((relationsStore, dzValue)=>{
                        const value = dzValue;
...
const validStrategy = (strategy)=>{
    assert$1(has('authenticate', strategy), INVALID_STRATEGY_MSG);
...
const createConfigProvider = (initialConfig = {}, strapi)=>{
    ...
    const transformDeprecatedPaths = (path)=>{
        ...
        if (isArray$1(path)) {
            ...
            if (path.some((part)=>!(isString$1(part) || isNumber(part)))) {
                return path;
            }
    ...
    return {
        ...
        get (path, defaultValue) {
            return get$2(state.config, transformDeprecatedPaths(path), defaultValue);
        },
        set (path, val) {
        ...
        has (path) {
            return has$1(state.config, transformDeprecatedPaths(path));
        }
    };
};
...
export { compile as compileStrapi, createStrapi, factories };
