 ...
 */ const build = async ({ logger, cwd, tsconfig, ...options })=>{
    ...
    try {
        ...
        if (ctx.bundler === 'webpack') {
            const { build: buildWebpack } = await import('./build-CrmGjqop.mjs');
            await buildWebpack(ctx);
        } else if (ctx.bundler === 'vite') {
            const { build: buildVite } = await import('./build-CzwYtqoc.mjs');
            await buildVite(ctx);
        }
...
const develop = async ({ cwd, polling, logger, tsconfig, watchAdmin, ...options })=>{
     ...
     */ let bundleWatcher;
        ...
        if (watchAdmin) {
            ...
            await writeStaticClientFiles(ctx);
            if (ctx.bundler === 'webpack') {
                const { watch: watchWebpack } = await import('./watch-DZEKM128.mjs');
                bundleWatcher = await watchWebpack(ctx);
            } else if (ctx.bundler === 'vite') {
                const { watch: watchVite } = await import('./watch-DwVyK07I.mjs');
...
//# sourceMappingURL=index-CVv0hWFI.mjs.map
