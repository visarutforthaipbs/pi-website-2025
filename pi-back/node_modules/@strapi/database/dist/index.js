 ...
 */ const createJoinTable = (metadata, { attributeName, attribute, meta })=>{
    ...
    const joinTableName = identifiers.getJoinTableName(_.snakeCase(meta.tableName), _.snakeCase(attributeName));
    const joinColumnName = identifiers.getJoinColumnAttributeIdName(_.snakeCase(meta.singularName));
    let inverseJoinColumnName = identifiers.getJoinColumnAttributeIdName(_.snakeCase(targetMeta.singularName));
    // if relation is self referencing
    if (joinColumnName === inverseJoinColumnName) {
        inverseJoinColumnName = identifiers.getInverseJoinColumnAttributeIdName(_.snakeCase(targetMeta.singularName));
    }
...
const createLifecyclesProvider = (db)=>{
    ...
    return {
        subscribe (subscriber) {
            assert.strict(isValidSubscriber(subscriber), 'Invalid subscriber. Expected function or object');
 ...
 */ const validateBidirectionalRelations = async (db)=>{
    ...
    for (const { relation, invRelation } of invalidLinks){
        ...
        const joinTableName = identifiers.getJoinTableName(_.snakeCase(modelMetadata.tableName), _.snakeCase(invRelation.inversedBy));
        const inverseJoinTableName = identifiers.getJoinTableName(_.snakeCase(invModelMetadata.tableName), _.snakeCase(relation.inversedBy));
