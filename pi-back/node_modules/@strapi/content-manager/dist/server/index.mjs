                ...
                case 'relation':
                    {
                        ...
                        const isVisible = contentTypes$1.isVisibleAttribute(model, attributeName);
                        if (isVisible) {
                            acc[attributeName] = {
                                [fieldSelector]: [
                                    'documentId',
            ...
            strapi1.documents.use(async (context, next)=>{
                ...
                const localeEntries = await strapi1.db.query(uid).findMany({
                    where: {
                        ...
                        ...contentTypes$1.hasDraftAndPublish(strapi1.contentTypes[uid]) ? {
                            publishedAt: null
                        } : {}
                    },
 ...
 */ var createModelConfigurationSchema = ((schema, opts = {})=>yup$1.object().shape({
...
const createSettingsSchema = (schema)=>{
    ...
    return yup$1.object().shape({
        ...
        // should be reset when the type changes
        defaultSortBy: yup$1.string().test('is-valid-sort-attribute', '${path} is not a valid sort attribute', async (value)=>isValidDefaultSort(schema, value)).default('id'),
        defaultSortOrder: yup$1.string().oneOf([
            'ASC',
            'DESC'
        ]).default('ASC')
    }).noUnknown();
...
const createMetadasSchema = (schema)=>{
    return yup$1.object().shape(Object.keys(schema.attributes).reduce((acc, key)=>{
...
const createLayoutsSchema = (schema, opts = {})=>{
    ...
    return yup$1.object().shape({
        edit: yup$1.array().of(yup$1.array().of(yup$1.object().shape({
...
const multipleLocaleSchema = yup$1.lazy((value)=>Array.isArray(value) ? yup$1.array().of(singleLocaleSchema.required()) : singleLocaleSchema);
const statusSchema = yup$1.mixed().oneOf([
    'draft',
    'published'
], 'Invalid status');
/**
 ...
 */ const getDocumentLocaleAndStatus = async (request, model, opts = {
    ...
    const defaultStatus = contentTypes$1.hasDraftAndPublish(strapi.getModel(model)) ? undefined : 'published';
 ...
 */ const createDocument = async (ctx, opts)=>{
    ...
    const setCreator = setCreatorFields({
        user
    });
 ...
 */ const updateDocument = async (ctx, opts)=>{
    ...
    const setCreator = documentVersion ? setCreatorFields({
        user,
        isEdition: true
    }) : setCreatorFields({
        user
   ...
   */ async publish (ctx) {
    ...
    async unpublish (ctx) {
        ...
        await strapi.db.transaction(async ()=>{
            ...
            ctx.body = await async.pipe((document)=>documentManager.unpublish(document.documentId, model, {
                    locale
    ...
    async discard (ctx) {
        ...
        ctx.body = await async.pipe((document)=>documentManager.discardDraft(document.documentId, model, {
                locale
            }), permissionChecker.sanitizeOutput, (document)=>formatDocumentWithMetadata(permissionChecker, model, document))(document);
    },
...
var relations = {
    async extractAndValidateRequestInfo (ctx, id) {
        ...
        if (!attribute || attribute.type !== 'relation') {
            throw new errors.ValidationError(`The relational field ${targetField} doesn't exist on ${model}`);
        }
        ...
        const mainField = flow(prop(`metadatas.${targetField}.edit.mainField`), (mainField)=>mainField || 'id', (mainField)=>sanitizeMainField(targetSchema, mainField, userAbility))(modelConfig);
   ...
   */ async findAvailable (ctx) {
     ...
     */ const dbQuery = strapi.db.query(sourceUid);
        const loadRelations = relations$1.isAnyToMany(attribute) ? (...args)=>dbQuery.loadPages(...args) : (...args)=>dbQuery.load(...args)// Ensure response is an array
            .then((res)=>({
                    results: res ? [
                        res
                    ] : []
...
var documentMetadata = (({ strapi })=>({
   ...
   */ async getAvailableLocales (uid, version, allVersions) {
            ...
            const model = strapi.getModel(uid);
            const mappingResult = await async.map(Object.values(versionsByLocale), async (localeVersions)=>{
   ...
   */ async formatDocumentWithMetadata (uid, document, opts = {}) {
            ...
            const hasDraftAndPublish = contentTypes$1.hasDraftAndPublish(strapi.getModel(uid));
